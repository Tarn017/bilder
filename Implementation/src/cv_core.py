...
def run_loto_cv(cfg: PatchCoreConfig, tools: list[str] | None = None,
    ...
    for tool in tools:
        test = [r for r in official if r["tool"] == tool]
        train = [r for r in official if r["tool"] != tool and not r["has_defect"]]
        if cfg.use_extras:
            train += [r for r in extras if r["tool"] != tool]
        if verbose:
        ...
        for r in test:
            ...
            oof.append({**r, "score": score, "fold": tool})
