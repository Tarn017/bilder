...
class PatchCoreConfig:
    backbone: str = "wide_resnet50_2"   # "resnet18" | "wide_resnet50_2" | "dinov2_vits14"
    input_size: int = 384               # durch 8 teilbar
    grayscale: bool = False
    clahe: bool = False
    aug_n: int = 3                      # 0..7 D4-Varianten (90er-Rotation/Spiegelung)
    aug_rot_n: int = 0                  # 0..4 Kleinwinkel-Rotationen (+-8/+-16 Grad)
    aug_photo_n: int = 0                # 0..4 photometrische Varianten (Helligkeit/Kontrast)
    ...
    use_synth: bool = False             # synthetische Gut-Bilder in die Bank
...
_ROT_ANGLES = [8, -8, 16, -16]
_PHOTO = [(1.0, 20), (1.0, -20), (1.2, 0), (0.85, 0)]   # (Kontrast alpha, Helligkeit beta)


def _small_rot(img: np.ndarray, angle: float) -> np.ndarray:
    h, w = img.shape[:2]
...
def _variants(img: np.ndarray, cfg) -> list[np.ndarray]:
    """Deterministische Augmentierungs-Varianten eines Normal-Bilds."""
    out = [img]
    out += [np.ascontiguousarray(f(img)) for f in _D4[:cfg.aug_n]]
    out += [_small_rot(img, a) for a in _ROT_ANGLES[:cfg.aug_rot_n]]
    out += [cv2.convertScaleAbs(img, alpha=al, beta=be)
            for al, be in _PHOTO[:cfg.aug_photo_n]]
    return out
...
class PatchCore:
    def __init__(self, cfg: PatchCoreConfig):
        ...
        self.is_vit = cfg.backbone.startswith("dinov2")
        if self.is_vit:
            # DINOv2-Patchgroesse 14: Eingabegroesse auf Vielfaches von 14 runden
            cfg.input_size = max(14, round(cfg.input_size / 14) * 14)
            net = torch.hub.load("facebookresearch/dinov2", cfg.backbone)
            net.eval()
            self.extractor = net.to(self.device)
        elif cfg.backbone == "resnet18":
            net = resnet18(weights="IMAGENET1K_V1")
            net.eval()
            self.extractor = create_feature_extractor(
                net, return_nodes={"layer2": "l2", "layer3": "l3"}).to(self.device)
    ...
    def _embed(self, imgs: list[np.ndarray]) -> torch.Tensor:
        ...
        if self.is_vit:
            with torch.no_grad():
                tokens = self.extractor.forward_features(x)["x_norm_patchtokens"]
            b, n, c = tokens.shape                      # (B, H*W, C)
            g = int(n ** 0.5)
            feat = tokens.permute(0, 2, 1).reshape(b, c, g, g)
            feat = F.avg_pool2d(feat, 3, 1, 1)          # lokale Glaettung wie beim CNN
            self.grid = (g, g)
            return feat.permute(0, 2, 3, 1).reshape(b, g * g, c)
        with torch.no_grad():
    ...
    def fit(self, image_paths: list) -> None:
        ...
        for p in image_paths:
            variants += _variants(self._load(p), cfg)
