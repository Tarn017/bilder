...
def main() -> None:
    ...
    # --- 2b) Synthetische Gut-Bilder (defec_synth/518_close) ----------------
    n_synth = 0
    synth_src = ROOT / "defec_synth" / "518_close"
    if synth_src.exists():
        (DATA / "synth").mkdir(exist_ok=True)
        for img in sorted(synth_src.glob("*.png")):
            shutil.copy2(img, DATA / "synth" / img.name)
            rows.append({
                "path": f"synth/{img.name}",
                "mask_path": "",
                "base_image": f"synth_{img.stem}",
                "tool": "synth",
                "view": "crop",
                "source": "synth_normal",
                "weak_label": 0,
    ...
    print(f"Synth-Gutbilder:     {n_synth}")
